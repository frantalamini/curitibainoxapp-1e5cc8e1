...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // A signed URL gerada usa a URL interna do Docker (http://kong:8000).
    // Substituir pela URL pública para que o browser consiga acessar.
    const internalBase = Deno.env.get("SUPABASE_URL") ?? "";
    const publicBase = Deno.env.get("SUPABASE_PUBLIC_URL") ?? "";
    const pdfUrl = publicBase
      ? signedData.signedUrl.replace(internalBase, publicBase)
