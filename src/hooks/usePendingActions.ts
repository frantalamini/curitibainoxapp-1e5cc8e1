  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const serviceCallIds = [
        ...new Set(messages.map((m) => m.service_call_id)),
      ];
      ...
      const validServiceCallIds = new Set(
        filteredServiceCalls.map((sc) => sc.id),
      );
      ...
      const pendingActions: PendingAction[] = messages
        ...
        .map((msg) => {
          const serviceCall = filteredServiceCalls.find(
            (sc) => sc.id === msg.service_call_id,
          );
          ...
          return {
            ...
            technician_name:
              (serviceCall?.technicians as any)?.full_name || "N/A",
