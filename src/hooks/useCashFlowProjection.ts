...
export const useCashFlowProjection = (
  ...
  const { data: transactions = [], isLoading: transactionsLoading } = useQuery({
    queryKey: [
      "cash-flow-projection-transactions",
      accountId,
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select(
          "id, amount, direction, status, due_date, paid_at, financial_account_id",
        )
        .neq("status", "CANCELED");
    ...
    useQuery({
      ...
      queryFn: async () => {
        let query = supabase
          ...
          .select(
            "id, amount, direction, day_of_month, is_active, start_date, end_date, financial_account_id",
          )
          .eq("is_active", true);
  ...
  const calculateProjectedBalances = (): ProjectedBalance[] => {
    ...
    const baseOpeningBalance = selectedAccounts.reduce(
    ...
    const recurringProjections = new Map<
      string,
    ...
    while (isBefore(currentMonth, addDays(endDate, 1))) {
      ...
      recurringTransactions.forEach((rt) => {
        ...
        const existsInTransactions = transactions.some(
          (t) =>
        ...
        const existing = recurringProjections.get(dueDateStr) || {
          income: 0,
          expense: 0,
        };
    ...
    while (isBefore(currentDate, addDays(endDate, 1))) {
      ...
      if (isProjected) {
        ...
        income = transactions
          .filter((t) => {
            if (t.direction !== "RECEIVE" || t.status !== "PAID" || !t.paid_at)
              return false;
        ...
        expense = transactions
          .filter((t) => {
            if (t.direction !== "PAY" || t.status !== "PAID" || !t.paid_at)
              return false;
            return t.paid_at.split("T")[0] === dayStr;
          })
          .reduce((sum, t) => sum + t.amount, 0);
  ...
  const firstNegativeDate =
    projectedBalances.find((p) => p.hasNegativeBalance)?.date || null;

  // Calculate summary
  const summary = {
    currentBalance:
      projectedBalances.find((p) => p.date === format(today, "yyyy-MM-dd"))
        ?.closingBalance ?? 0,
    projectedEndBalance:
      projectedBalances[projectedBalances.length - 1]?.closingBalance ?? 0,
    totalProjectedIncome: projectedBalances
      .filter((p) => p.isProjected)
      .reduce((sum, p) => sum + p.income, 0),
    totalProjectedExpense: projectedBalances
      .filter((p) => p.isProjected)
