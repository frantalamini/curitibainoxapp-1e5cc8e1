...
export const useCashFlow = (
  ...
  const calculateDailyBalances = (): DailyBalance[] => {
    ...
    while (isBefore(currentDate, addDays(endDate, 1))) {
      ...
      accumulatedBalance =
        accumulatedBalance + realizedIncome - realizedExpense;
  ...
  const summary = {
    ...
    totalExpectedIncome: dailyBalances.reduce(
      (sum, d) => sum + d.expectedIncome,
      0,
    ),
    totalExpectedExpense: dailyBalances.reduce(
      (sum, d) => sum + d.expectedExpense,
      0,
    ),
    totalRealizedIncome: dailyBalances.reduce(
      (sum, d) => sum + d.realizedIncome,
      0,
    ),
    totalRealizedExpense: dailyBalances.reduce(
      (sum, d) => sum + d.realizedExpense,
      0,
    ),
    finalExpectedBalance:
      dailyBalances[dailyBalances.length - 1]?.expectedClosing ?? 0,
    finalRealizedBalance:
      dailyBalances[dailyBalances.length - 1]?.realizedClosing ?? 0,
  };
