...
export const useDREData = (month: number, year: number) => {
  ...
  const totals: DRETotals = {
    totalIncomeExpectedMonth: incomeSummaries.reduce(
      (sum, c) => sum + c.expectedMonth,
      0,
    ),
    totalIncomeRealizedMonth: incomeSummaries.reduce(
      (sum, c) => sum + c.realizedMonth,
      0,
    ),
    totalExpenseExpectedMonth: expenseSummaries.reduce(
      (sum, c) => sum + c.expectedMonth,
      0,
    ),
    totalExpenseRealizedMonth: expenseSummaries.reduce(
      (sum, c) => sum + c.realizedMonth,
      0,
    ),
    netExpectedMonth: 0,
    netRealizedMonth: 0,
    totalIncomeExpectedYear: incomeSummaries.reduce(
      (sum, c) => sum + c.expectedYear,
      0,
    ),
    totalIncomeRealizedYear: incomeSummaries.reduce(
      (sum, c) => sum + c.realizedYear,
      0,
    ),
    totalExpenseExpectedYear: expenseSummaries.reduce(
      (sum, c) => sum + c.expectedYear,
      0,
    ),
    totalExpenseRealizedYear: expenseSummaries.reduce(
      (sum, c) => sum + c.realizedYear,
      0,
    ),
    netExpectedYear: 0,
  ...
  totals.netExpectedMonth =
    totals.totalIncomeExpectedMonth - totals.totalExpenseExpectedMonth;
  totals.netRealizedMonth =
    totals.totalIncomeRealizedMonth - totals.totalExpenseRealizedMonth;
  totals.netExpectedYear =
    totals.totalIncomeExpectedYear - totals.totalExpenseExpectedYear;
  totals.netRealizedYear =
    totals.totalIncomeRealizedYear - totals.totalExpenseRealizedYear;
