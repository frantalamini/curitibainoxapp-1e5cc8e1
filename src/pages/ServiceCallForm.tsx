...
import { ArrivalDetectionBanner } from "@/components/ArrivalDetectionBanner";
import { useServiceCallStatusLog } from "@/hooks/useServiceCallStatusLog";
import { StatusTimeline } from "@/components/service-calls/StatusTimeline";
...
const ServiceCallForm = () => {
  ...
  const { data: profilePerms, isLoading: permsLoading } =
    useCurrentUserProfilePermissions();
  const isGerencial = profilePerms?.isGerencial ?? false;
  const canViewFinancial = hasProfilePermission(
    profilePerms,
    "os_aba_financeiro",
    "can_consult",
  );
  ...
  const { data: statusLogs = [], isLoading: isLoadingStatusLogs } =
    useServiceCallStatusLog(id);
  const {
    createTrip,
    updateTrip,
  ...
  const { nearDestination } = useGpsTracking(activeTrip);

  const handleAutoArrive = useCallback(() => {
    ...
    updateTrip({ id: activeTrip.id, updates });
  ...
  const {
    ...
    dismiss: dismissArrival,
  } = useAutoArrival({
    nearDestination,
    activeTrip,
    onAutoArrive: handleAutoArrive,
  });
  ...
  const [existingPhotosBeforeUrls, setExistingPhotosBeforeUrls] = useState<
    string[]
  >([]);
  const [existingVideoBeforeUrl, setExistingVideoBeforeUrl] = useState<
    string | null
  >(null);
  const [photosAfterFiles, setPhotosAfterFiles] = useState<File[]>([]);
  const [videoAfterFile, setVideoAfterFile] = useState<File | null>(null);
  const [existingPhotosAfterUrls, setExistingPhotosAfterUrls] = useState<
    string[]
  >([]);
  const [existingVideoAfterUrl, setExistingVideoAfterUrl] = useState<
    string | null
  >(null);
  ...
  const [selectedCommercialStatusId, setSelectedCommercialStatusId] =
    useState<string>("");
  ...
  const [existingTechnicianSignatureUrl, setExistingTechnicianSignatureUrl] =
    useState<string | null>(null);
  const [existingCustomerSignatureUrl, setExistingCustomerSignatureUrl] =
    useState<string | null>(null);
  ...
  const [openClientSignatureModal, setOpenClientSignatureModal] =
    useState(false);
  const [newSignatures, setNewSignatures] = useState<Signature[]>([]);
  const [resolvedSigUrls, setResolvedSigUrls] = useState<{
    tech?: string | null;
    client?: string | null;
  }>({});

  // Estados para gravação de áudio de informações técnicas
  const [technicalAudioFile, setTechnicalAudioFile] = useState<File | null>(
    null,
  );
  const [existingTechnicalAudioUrl, setExistingTechnicalAudioUrl] = useState<
    string | null
  >(null);
  const [isRecordingTechnical, setIsRecordingTechnical] = useState(false);
  const [technicalMediaRecorder, setTechnicalMediaRecorder] =
    useState<MediaRecorder | null>(null);
  const [technicalAudioBlob, setTechnicalAudioBlob] = useState<Blob | null>(
    null,
  );
  ...
  const commercialStatusName =
    allStatuses
      ?.find((s) => s.id === selectedCommercialStatusId)
      ?.name?.toLowerCase() || "";
  const isStatusFaturado =
    !!selectedCommercialStatusId && commercialStatusName === "faturado";
  const isStatusConcluido =
    !!selectedCommercialStatusId && commercialStatusName === "concluído";
  ...
  const getCurrentSignature = (
    existingSignatures: any[] | undefined,
    newSigs: Signature[],
    role: "tech" | "client",
  ) => {
    // Combinar assinaturas existentes com novas ainda não salvas
    const allSignatures = [...(existingSignatures || []), ...newSigs].filter(
      (s: any) => s.role === role,
    );

    if (allSignatures.length === 0) return null;
    return allSignatures.sort(
      (a: any, b: any) =>
  ...
  const currentTechSignature = getCurrentSignature(
    (existingCall as any)?.signatures,
    newSignatures,
    "tech",
  );
  const currentClientSignature = getCurrentSignature(
    (existingCall as any)?.signatures,
    newSignatures,
    "client",
  );

  // Resolve URLs das assinaturas para exibição (converte path relativo → signed URL)
  useEffect(() => {
    ...
    const resolve = async () => {
      const [tech, client] = await Promise.all([
        resolveUrl(currentTechSignature?.image_url),
        resolveUrl(currentClientSignature?.image_url),
      ]);
      setResolvedSigUrls({ tech, client });
    };
  ...
  const handleStartTrip = async (data: {
    ...
    if (data.tripType === "interno") {
      // Serviço interno — sem veículo, sem GPS
      createTrip({
        service_call_id: id,
    ...
    // Deslocamento normal — fluxo original
    ...
    createTrip({
      ...
      trip_type: "deslocamento",
      origin_lat: data.originLat ?? undefined,
      origin_lng: data.originLng ?? undefined,
      ...
      current_lat: data.originLat ?? undefined,
      current_lng: data.originLng ?? undefined,
    });
          ...
          if (isIOS) {
            ...
            if (!newWindow) {
              toast({
                ...
                description:
                  "O navegador bloqueou a abertura do GPS. Permita pop-ups para este site e tente novamente.",
              });
            }
          }
        }, 300);
  ...
  const handleEndTrip = async (endOdometer: number | null) => {
    ...
    // Se quilometragem informada e tem veículo, calcular distância e atualizar veículo
    if (endOdometer !== null && activeTrip.vehicle_id) {
  ...
  useEffect(() => {
    if (isEditMode) {
      const checkFinanceiroTab = () => {
        const financeiroTrigger = document.querySelector(
          '[data-testid="financeiro-tab-trigger"]',
        );
        const financeiroContent = document.querySelector(
          '[data-testid="financeiro-tab-content"]',
        );
  ...
  useEffect(() => {
    if (existingCall && isEditMode && !initializedRef.current) {
      ...
      setValue(
        ...
        (existingCall as any).equipment_manufacturer || "",
      );
      setValue(
        "equipment_sector",
        (existingCall as any).equipment_sector || "",
      );
      ...
      setExistingTechnicianSignatureUrl(
        existingCall.technician_signature_url || null,
      );
      setExistingCustomerSignatureUrl(
        existingCall.customer_signature_url || null,
      );
      ...
      setExistingTechnicalAudioUrl(
        existingCall.technical_diagnosis_audio_url || null,
      );
    ...
    const isTechBlocked =
      !canViewFinancial && (existingCall as any)?.has_financial_report;
    if (
      existingCall &&
      isEditMode &&
      (existingCall as any).report_access_token &&
      !isTechBlocked
    ) {
  ...
  const startRecordingTechnical = async () => {
    try {
      ...
      recorder.onstop = () => {
        ...
        const file = new File(
          [blob],
          `technical-audio-${Date.now()}.${extension}`,
          {
            type: mimeType,
          },
        );
  ...
  const stopRecordingTechnical = () => {
    if (
      technicalMediaRecorder &&
      technicalMediaRecorder.state === "recording"
    ) {
      technicalMediaRecorder.stop();
      technicalMediaRecorder.stream
        .getTracks()
  ...
  // Alias para função centralizada de compressão (importada no topo do arquivo)
  const ensureJpegFile = compressImageFile;
  ...
  const onSubmit = async (data: ServiceCallInsert) => {
    ...
    const effectiveClientId =
      isGeralReadonly && isEditMode && existingCall
    ...
    if (isEditMode && !defectFound.trim()) {
      toast({
        title: "Erro",
        description:
          "Preencha o campo 'Defeito Encontrado' na aba Informações Técnicas",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      let technicianSignatureUrl: string | null =
        existingTechnicianSignatureUrl;
      ...
      if (mediaFiles.length > 0) {
        for (const rawFile of mediaFiles) {
          const file = await ensureJpegFile(rawFile);
      ...
      if (photosBeforeFiles.length > 0) {
        for (const rawFile of photosBeforeFiles) {
          const file = await ensureJpegFile(rawFile);
          const fileExt = file.name.split(".").pop();
          ...
          if (error) {
            console.error("Erro ao fazer upload de foto (antes):", error);
            toast({
              title: "Erro ao salvar foto",
              description: `Não foi possível salvar "${file.name}". Verifique o tamanho do arquivo e tente novamente.`,
              variant: "destructive",
            });
          } else {
            const { data: signedData } = await supabase.storage
            ...
            if (signedData?.signedUrl)
              photosBeforeUrls.push(signedData.signedUrl);
          }
        }
      }
      ...
      if (videoBeforeFile) {
        ...
        if (error) {
          console.error("Erro ao fazer upload de vídeo (antes):", error);
          toast({
            title: "Erro ao salvar vídeo",
            description:
              "Não foi possível salvar o vídeo. Verifique o tamanho e tente novamente.",
            variant: "destructive",
          });
        } else {
          const { data: signedData } = await supabase.storage
      ...
      if (photosAfterFiles.length > 0) {
        for (const rawFile of photosAfterFiles) {
          const file = await ensureJpegFile(rawFile);
          const fileExt = file.name.split(".").pop();
          ...
          if (error) {
            console.error("Erro ao fazer upload de foto (depois):", error);
            toast({
              title: "Erro ao salvar foto",
              description: `Não foi possível salvar "${file.name}". Verifique o tamanho do arquivo e tente novamente.`,
              variant: "destructive",
            ...
            const { data: signedData } = await supabase.storage
              ...
              .createSignedUrl(filePath, 31536000);
            if (signedData?.signedUrl)
              photosAfterUrls.push(signedData.signedUrl);
          }
        }
      }
      ...
      if (videoAfterFile) {
        ...
        if (error) {
          console.error("Erro ao fazer upload de vídeo (depois):", error);
          toast({
            title: "Erro ao salvar vídeo",
            description:
              "Não foi possível salvar o vídeo. Verifique o tamanho e tente novamente.",
      ...
      if (technicalAudioFile) {
        ...
        if (audioError) {
          ...
          toast({
            title: "Erro",
            description:
              "Erro ao fazer upload do áudio de informações técnicas",
            variant: "destructive",
          });
      ...
      const processedSignatures: Signature[] = [
        ...((existingCall as any)?.signatures || []),
      ];
      ...
      const formattedData: any = {
        ...
        photos_before_urls:
          photosBeforeUrls.length > 0 ? photosBeforeUrls : null,
        ...
        technician_signature_data:
          (existingCall as any)?.technician_signature_data || null,
        technician_signature_url:
          (existingCall as any)?.technician_signature_url || null,
        customer_signature_data:
          (existingCall as any)?.customer_signature_data || null,
        customer_signature_url:
          (existingCall as any)?.customer_signature_url || null,
        ...
        technician_signature_date:
          (existingCall as any)?.technician_signature_date || null,
        customer_signature_date:
          (existingCall as any)?.customer_signature_date || null,
      };
      ...
      if (isEditMode && id) {
        try {
          ...
          if (
            updatedCall?.report_pdf_path &&
            updatedCall?.report_access_token
          ) {
            setGeneratedPdfUrl(
              `https://curitibainoxapp.com/relatorio-os/${updatedCall.os_number}/${updatedCall.report_access_token}`,
            );
          }
  ...
  return (
    <MainLayout>
      <div className="w-full max-w-[1400px] mr-auto pl-2 pr-6 sm:pl-3 sm:pr-8 lg:pl-4 lg:pr-10 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {isFaturado && (
              ...
              >
                {isStatusFaturado ? "OS Faturada" : "OS Concluída"} — Edição
                bloqueada
              </Badge>
            )}
            ...
            <CardContent className="p-4">
              ...
              {activeTrip && activeTrip.started_at && (
                <div className="space-y-3">
                  ...
                  >
                    <Car
                      className={`h-4 w-4 ${activeTrip.trip_type === "interno" ? "text-purple-600" : "text-amber-600"}`}
                    />
                    <AlertDescription
                      className={
                        activeTrip.trip_type === "interno"
                  ...
                  {arrivalDetected && activeTrip.trip_type !== "interno" && (
                    <ArrivalDetectionBanner
        ...
        {isEditMode && isGerencial && (
          <StatusTimeline logs={statusLogs} isLoading={isLoadingStatusLogs} />
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      O checklist selecionado estará disponível na aba
                        ...
                        mediaPreviews.map((preview, index) => (
                          <div key={index} className="relative">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                setMediaFiles((prev) =>
                                  prev.filter((f) => f !== preview.file),
                                );
                                setMediaPreviews((prev) =>
                                  prev.filter((p) => p.file !== preview.file),
                                );
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground mb-2">
                          Anotações visíveis apenas para administradores e
                          técnicos. Não serão incluídas nos relatórios enviados
                          ao cliente.
                        </p>
                        <Textarea
                ...
                <CardContent className="space-y-6">
                  ...
                  {isEditMode && !hasCompletedTrip && (
                    <Alert variant="destructive">
                      ...
                      <AlertDescription>
                        Para preencher as informações técnicas, finalize
                        primeiro o deslocamento clicando em "Cheguei no Cliente"
                        no banner de Deslocamento acima.
                      </AlertDescription>
                    </Alert>
                  )}
                        ...
                        {currentTechSignature ? (
                          <div className="space-y-2">
                              ...
                              className={`border-2 rounded-lg p-4 ${
                                newSignatures.some(
                                  (s) =>
                                    s.role === "tech" &&
                                    s.image_url ===
                                      currentTechSignature.image_url,
                                )
                            ...
                            <p className="text-xs text-muted-foreground">
                              ...
                              Assinado em{" "}
                              {format(
                                new Date(currentTechSignature.signed_at),
                                "dd/MM/yyyy 'às' HH:mm",
                                { locale: ptBR },
                              )}
                              {currentTechSignature.signed_by &&
                                ` • ${currentTechSignature.signed_by}`}
                            </p>
                          </div>
                        ) : (
                          ...
                          <div className="space-y-2">
                              ...
                              className={`border-2 rounded-lg p-4 ${
                                newSignatures.some(
                                  (s) =>
                                    s.role === "client" &&
                                    s.image_url ===
                                      currentClientSignature.image_url,
                                )
                                  ? "border-primary bg-primary/5"
                                  : "border-muted bg-muted/10"
                              }`}
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-sm font-medium">
                                {currentClientSignature.signed_by}
                              </p>
                              {currentClientSignature.position && (
                                <p className="text-xs text-muted-foreground">
                                  Cargo: {currentClientSignature.position}
                                </p>
                              )}
                              <p className="text-xs text-muted-foreground">
                                Assinado em{" "}
                                {format(
                                  new Date(currentClientSignature.signed_at),
                                  "dd/MM/yyyy 'às' HH:mm",
                                  { locale: ptBR },
                                )}
                              </p>
          ...
          <div className="flex gap-4 mt-6 flex-wrap">
            ...
            {isEditMode && existingCall && (
              <>
                {/* PDF Técnico: sempre disponível para todos */}
                <Button
                  type="button"
                  variant="outline"
                  onClick={async () => {
                    try {
                      ...
                      const { blob, fileName, blobUrl } = await generateOSPdf(
                        existingCall.id,
                      ...
                      toast({
                        ...
                        description:
                          "Use os botões abaixo para visualizar, salvar ou enviar.",
                      });
                    } catch (error: any) {
                            ...
                            if (pdfBlobUrl) {
                              ...
                              const { data: signedData, error: signedError } =
                                await supabase.storage
                                  .from("service-call-attachments")
                                  .createSignedUrl(
                                    (existingCall as any).report_pdf_path,
                                    3600,
                                  );
        ...
        <SignatureModal
          ...
          onSave={(dataUrl) => {
            const technicianName =
              activeTechnicians?.find((t) => t.id === selectedTechnicianId)
                ?.full_name || "Técnico";
          ...
          <>
            ...
            {activeTrip && activeTrip.start_odometer_km !== undefined && (
              <EndTripModal
                ...
                isInternal={activeTrip.trip_type === "interno"}
              />
            )}
          </>
        )}
