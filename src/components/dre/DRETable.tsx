    ...
    if (categories.length === 0) {
      return (
        <TableRow>
          <TableCell
            colSpan={5}
            className="text-center text-muted-foreground py-2 text-sm"
          >
        ...
        <TableRow key={cat.categoryId} className="hover:bg-muted/30">
          ...
          <TableCell
            className={cn(
              ...
              getVarianceColor(budget, realized, isExpense),
            )}
          >
      ...
      <TableRow className={cn(bgClass, "font-semibold")}>
        ...
        <TableCell
          className={cn(
            "text-right font-mono",
            isResult
              ? getVarianceColor(budget, realized, false)
              : getVarianceColor(budget, realized, isExpense),
          )}
        >
          {formatCurrency(realized - budget)}
  ...
  return (
    <Table>
      ...
      <TableBody>
        ...
        {renderSectionHeader(
          "2. Custo da Mercadoria Produzida/Vendida",
          "bg-orange-50 dark:bg-orange-950/30",
        )}
        {renderCategoryRows(
          [DRE_GROUPS.CMV_MERCADORIAS, DRE_GROUPS.CMV_SERVICOS],
          true,
        )}
        {renderSubtotalRow(
          "Total CMV",
        ...
        <TableRow className="bg-blue-100 dark:bg-blue-900/30 font-bold">
          ...
          <TableCell className="text-right font-mono">
            {formatCurrency(getBudget(margemContribuicao))}
          </TableCell>
          <TableCell className="text-right font-mono">
            {formatCurrency(getRealized(margemContribuicao))}
          </TableCell>
          <TableCell
            className={cn(
              "text-right font-mono",
              getVarianceColor(
                getBudget(margemContribuicao),
                getRealized(margemContribuicao),
              ),
            )}
          >
            {formatCurrency(
              getRealized(margemContribuicao) - getBudget(margemContribuicao),
            )}
          </TableCell>
        ...
        <TableRow className="bg-blue-50 dark:bg-blue-950/20">
          ...
          <TableCell className="text-right font-mono">
            {formatPercent(
              showMonthly
                ? margemContribuicaoPct.budgetMonth
                : margemContribuicaoPct.budgetYear,
            )}
          </TableCell>
          <TableCell className="text-right font-mono">
            {formatPercent(
              showMonthly
                ? margemContribuicaoPct.realizedMonth
                : margemContribuicaoPct.realizedYear,
            )}
          </TableCell>
          <TableCell className="text-right font-mono">-</TableCell>
          <TableCell className="text-right font-mono">-</TableCell>
        ...
        >
          ...
          <TableCell className="text-right font-mono">
            {formatCurrency(getBudget(resultadoGlobal))}
          </TableCell>
          <TableCell className="text-right font-mono">
            {formatCurrency(getRealized(resultadoGlobal))}
          </TableCell>
          <TableCell
            className={cn(
              "text-right font-mono",
              getVarianceColor(
                getBudget(resultadoGlobal),
                getRealized(resultadoGlobal),
              ),
            )}
          >
            {formatCurrency(
              getRealized(resultadoGlobal) - getBudget(resultadoGlobal),
            )}
          </TableCell>
          <TableCell className="text-right font-mono">-</TableCell>
        </TableRow>
        ...
        <TableRow className="bg-slate-200 dark:bg-slate-700/40 font-semibold">
          ...
          <TableCell className="text-right font-mono">
            {formatCurrency(
              showMonthly
                ? pontoEquilibrio.budgetMonth
                : pontoEquilibrio.budgetYear,
            )}
          </TableCell>
          <TableCell className="text-right font-mono">
            {formatCurrency(
              showMonthly
                ? pontoEquilibrio.realizedMonth
                : pontoEquilibrio.realizedYear,
            )}
          </TableCell>
