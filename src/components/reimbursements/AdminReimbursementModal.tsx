...
interface AdminReimbursementModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

interface OpenServiceCall {
  id: string;
  os_number: number;
...
export function AdminReimbursementModal({
  open,
  onOpenChange,
}: AdminReimbursementModalProps) {
  ...
  const { data: allServiceCalls, isLoading: isLoadingCalls } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as OpenServiceCall[];
    },
    enabled: open,
  });
  ...
  // Limpar OS quando mudar técnico
  useEffect(() => {
    setSelectedServiceCallId("");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const fileName = `reimbursements/${selectedServiceCallId}/${Date.now()}_${photoFile.name}`;
      ...
      const input: CreateReimbursementInput = {
        service_call_id: selectedServiceCallId,
              ...
              <SelectContent>
                ...
                {filteredServiceCalls?.length === 0 && (
                  <div className="px-2 py-4 text-center text-sm text-muted-foreground">
                    Nenhuma OS aberta para este técnico
                  </div>
                )}
              </SelectContent>
            </Select>
