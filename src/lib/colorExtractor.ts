        ...
        for (let i = 0; i < pixels.length; i += 4) {
          ...
          for (const bucket of colorBuckets) {
            if (colorDistance({ r, g, b }, bucket) < threshold) {
              // Average the colors
              bucket.r = Math.round(
                (bucket.r * bucket.count + r) / (bucket.count + 1),
              );
              bucket.g = Math.round(
                (bucket.g * bucket.count + g) / (bucket.count + 1),
              );
              bucket.b = Math.round(
                (bucket.b * bucket.count + b) / (bucket.count + 1),
              );
              bucket.count++;
              found = true;
              break;
...
function generatePaletteFromBuckets(buckets: ColorBucket[]): ExtractedColor[] {
  ...
  const primaryBucket =
    buckets.find((b) => !isTooGray(b.r, b.g, b.b)) || buckets[0];
  if (primaryBucket) {
    usedColors.push(primaryBucket);
    result.push(
      createExtractedColor(primaryBucket, roles[0], primaryBucket.count),
    );
  }

  // Get secondary color (second most frequent, distinct from primary)
  const secondaryBucket = buckets.find(
    (b) =>
      !usedColors.some((u) => colorDistance(b, u) < 60) &&
      !isTooGray(b.r, b.g, b.b),
  );
  if (secondaryBucket) {
    usedColors.push(secondaryBucket);
    result.push(
      createExtractedColor(secondaryBucket, roles[1], secondaryBucket.count),
    );
  } else {
  ...
  const accentBucket = buckets.find(
    (b) =>
      !usedColors.some((u) => colorDistance(b, u) < 60) &&
  ...
  if (accentBucket) {
    ...
    result.push(
      createExtractedColor(accentBucket, roles[2], accentBucket.count),
    );
  } else {
  ...
  const successBucket = buckets.find(
    (b) =>
      b.g > b.r &&
      b.g > b.b &&
      !usedColors.some((u) => colorDistance(b, u) < 40),
  );
  if (successBucket) {
    result.push(
      createExtractedColor(successBucket, roles[3], successBucket.count),
    );
  } else {
  ...
  const warningBucket = buckets.find(
    (b) =>
      b.r > b.b &&
      b.g > b.b * 0.8 &&
      !usedColors.some((u) => colorDistance(b, u) < 40),
  ...
  if (warningBucket) {
    result.push(
      createExtractedColor(warningBucket, roles[4], warningBucket.count),
    );
  } else {
  ...
  const destructiveBucket = buckets.find(
    (b) =>
      b.r > b.g &&
      b.r > b.b &&
      !usedColors.some((u) => colorDistance(b, u) < 40),
